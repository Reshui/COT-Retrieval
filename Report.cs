...
public partial class Report
{
    ...
    /// <summary>
    /// Standardized field name for C.O.T date fields.
    /// </summary>
    private const string s_standardDateFieldName = "report_date_as_yyyy_mm_dd";

    /// <summary>
    /// Standardized field name for C.O.T contract code fields between the CFTC and ICE C.O.T
    /// </summary>
    private const string s_contractCodeColumnName = "cftc_contract_market_code";

    /// <summary>
    /// Standardized date format used to parse CFTC dates or convert ICE dates.
    /// </summary>
    ...
    public Report(ReportType queriedReport, bool retrieveCombinedData, string databasePath, string tableName)
    {
    ...
    /// <param name="testMode">Used to test certain functions. Use with caution</param>
    /// <param name="testUpload">If <see langword="true"/> and <paramref name="testMode"/> is <see langword="true"/> then data upload will be tested.</param>
    ...
    public async Task<bool> RetrieveDataAsync(Dictionary<string, string>? priceSymbolByContractCode, bool testMode = false, bool testUpload = false)
    {
        ...
        int queryReturnLimit = testMode ? 1 : 20_000;
        // https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/MIME_types
        const string outputType = ".csv", mimeType = "text/csv";
        ...
        List<string[]> cftcData = new();
        Dictionary<string, Dictionary<DateTime, string?>> pricesPerContractCode = new();
        ...
        try
        {
            ...
            {
                string? response;
                string comparisonOprator = testMode ? ">=" : ">";
                ...
                if (totalRecordsToRetrieve == 0)
                {
                    ...
                    {
                        totalRecordsToRetrieve = queryReturnLimit;
                    }
                ...
                if (!errorStatus && response != null)
                {
                    ...
                    int cftcDateColumn = (int)fieldInfoByEditedName[s_standardDateFieldName]?.ColumnIndex!;
                    int cftcCodeColumn = (int)fieldInfoByEditedName[s_contractCodeColumnName]?.ColumnIndex!;
                    ...
                    for (var i = 1; i < responseLines.Length; ++i)
                    {
                        if (!string.IsNullOrEmpty(responseLines[i]))
                        {
                            ...
                            && ((parsedDate > DataBaseDateBeforeUpdate && !testMode) || (parsedDate >= DataBaseDateBeforeUpdate && testMode)))
                            {   // Create a null entry for the current combination of contract code and date within pricesPerContractCode.
                                if (IsLegacyCombined)
                                {
                                    string currentContractCode = apiRecord[cftcCodeColumn];
                                    if (!pricesPerContractCode.TryGetValue(currentContractCode, out Dictionary<DateTime, string?>? priceByDate))
                                    {
                                        priceByDate = pricesPerContractCode[currentContractCode] = new();
                                    }
            ...
            } while (recordsQueriedCount < totalRecordsToRetrieve && !errorStatus);

            List<string[]>? iceData = null;

            #region ICE Retrieval if Disaggregated.       
            if (QueriedReport == ReportType.Disaggregated && cftcData.Any() && !errorStatus)
            {
                ...
                if (maxIceDateInDatabase < cftcApiMaxDate || testMode)
                {
                    ...
                    else if (!singleWeekRetrieval)
                    {
                        ...
                        for (var csvYear = maxIceDateInDatabase.Year; csvYear <= cftcApiMaxDate.Year; ++csvYear)
                        {
                            ...
                            {
                                s_iceCsvRawData.TryUpdate(iceCsvUrl, QueryIceDataAsync(iceCsvUrl, databaseFieldNames!, testMode), null);
                            }
                        }
                    }
                    ...
                    var iceQuery = from kvp in s_iceCsvRawData
                                   ...
                                   from dataByDateTime in kvp.Value.Result
                                   where (!testMode && dataByDateTime.Key > maxIceDateInDatabase) || (testMode && dataByDateTime.Key >= maxIceDateInDatabase)
                                   from row in dataByDateTime.Value
                                   where row[futOrCombinedColumn].ToLower().Equals(RetrieveCombinedData ? "combined" : "futonly")
                                   select row;

                    if (testMode) iceData = iceQuery.Take(queryReturnLimit).ToList();
                    else iceData = iceQuery.ToList();
                }
            }
            #endregion

            if (!errorStatus && cftcData.Any())
            {
                if (IsLegacyCombined && priceSymbolByContractCode != null)
                {
                    ...
                    if (retrievePrices) await RetrieveYahooPriceDataAsync(priceSymbolByContractCode, pricesPerContractCode);
                }

                if (!testMode || testUpload)
                {
                    CurrentStatus = ReportStatusCode.AttemptingUpdate;

                    errorStatus = !UploadToDatabase(fieldInfoByEditedName!, cftcData, uploadingIceData: false, priceData: pricesPerContractCode, currentlyDebugging: testUpload);
                    if (QueriedReport == ReportType.Disaggregated && iceData != null && iceData.Any())
                    {
                        errorStatus = !UploadToDatabase(s_iceColumnMap!, iceData, uploadingIceData: true, currentlyDebugging: testUpload);
                    }
    ...
    private static async Task<Dictionary<DateTime, List<string[]>>> QueryIceDataAsync(string iceCsvUrl, List<string> databaseHeaders, bool currentlyDebugging)
    {
    ...
    /// <param name="dataToUpload">Array of field separated values for each row of data.</param>
    ...
    /// <param name="currentlyDebugging"><see langword="true"/> if dates should be replaced with <see cref="s_defaultStartDate"/>; otherwise, <see langword="false"/>.</param>  
    /// <returns><see langword="true"/> if no errors are generated; otherwise, <see langword="false"/>.</returns>
    bool UploadToDatabase(Dictionary<string, FieldInfo?> fieldInfoPerEditedName, List<string[]> dataToUpload, bool uploadingIceData, Dictionary<string, Dictionary<DateTime, string?>>? priceData = null, bool currentlyDebugging = false)
    {
        ...
        lock (DatabaseConnection)
        {
            try
            {
                ...
                int cotCodeColumn = (int)fieldInfoPerEditedName[s_contractCodeColumnName]?.ColumnIndex!;
                int cotDateColumn = (int)fieldInfoPerEditedName[s_standardDateFieldName]?.ColumnIndex!;
                    ...
                    foreach (OleDbParameter param in cmd.Parameters)
                    {
                        ...
                        else
                        {
                            param.Value = param.OleDbType switch
                            {
                                ...
                                OleDbType.Date => currentlyDebugging ? s_defaultStartDate : DateTime.ParseExact(fieldValue, StandardDateFormat, CultureInfo.InvariantCulture, DateTimeStyles.None),
                                _ => throw new ArgumentOutOfRangeException($"Unaccounted for OleDbType: {param.OleDbType}")
                            };
                        }
                    }
                    cmd.ExecuteNonQuery();
    ...
    /// <summary>
    /// Asynchronously queries yahoo finance for price data and updates dictionaries within <paramref name="pricesPerContractCode"/>.
    /// </summary>
    /// <param name="priceSymbolByContractCode">Dictionary of price symbols keyed to cftc contract codes.</param>
    /// <param name="pricesPerContractCode">A contract code keyed dictionary that contains a dictionary keyed to wanted dates for the given contract code.</param>
    ...
    static async Task RetrieveYahooPriceDataAsync(Dictionary<string, string> priceSymbolByContractCode, Dictionary<string, Dictionary<DateTime, string?>> pricesPerContractCode)
    {
        ...
        {
            if (pricesPerContractCode.TryGetValue(knownSymbol.Key, out Dictionary<DateTime, string?>? wantedPrices))
            {
    ...
    {
        if (filterForIce && QueriedReport != ReportType.Disaggregated) throw new InvalidOperationException("If filtering for ice data then report type must be disaggregated.");
    ...
    static Dictionary<string, FieldInfo?> MapHeaderFieldsToDatabase(string[] externalHeaders, List<string> databaseFields, bool iceHeaders)
    {
        ...
        for (var i = 0; i < externalHeaders.Length; ++i)
        {
            ...
            {
                externalHeaders[i] = header.Contains("yyyy", StringComparison.InvariantCultureIgnoreCase) ? s_standardDateFieldName : header;
            }
        ...
        for (var i = 0; i < databaseFields.Count; ++i)
        {
            ...
            {
                editedTableFieldName = s_standardDateFieldName;
            }
            else
            {
        ...
        if (iceHeaders && headerIndexesByEditedName.ContainsKey(s_contractCodeColumnName) && headerIndexesByEditedName.Remove("cftc_commodity_code", out int columnIndex))
        {
            headerIndexesByEditedName[s_contractCodeColumnName] = columnIndex;
        }
        /*
